...
interface SyncState {
  lastSyncHead: string;
  spacePath: string;
  files: Record<string, { localHash: string; spaceSha: string }>;
}

interface LinkMapping {
  // local filename (e.g. "Purpose.md") → space node_id (e.g. "n_abc123")
  byLocalName: Map<string, string>;
  // space path (e.g. "starwatcher/docs/purpose.md") → node_id
 ...
 */
function buildLinkMapping(outline: OutlineItem[], spacePath: string): LinkMapping {
  const byLocalName = new Map<string, string>();
  const bySpacePath = new Map<string, string>();

  for (const item of outline) {
    if (!item.node_id || !item.path) continue;
...
export function resolveLinks(
  ...
  localDir: string,
  spacePath: string,
  siblingSpacePath: string,
): string {
  // Match markdown links: [text](target)
  return content.replace(
    /\[([^\]]*)\]\(([^)]+)\)/g,
    (match, text, target: string) => {
...
async function loadSyncState(localPath: string): Promise<SyncState | null> {
  try {
...
async function saveSyncState(localPath: string, state: SyncState): Promise<void> {
  await writeFile(
    join(localPath, SYNC_STATE_FILE),
...
async function collectLocalFiles(dirPath: string): Promise<Map<string, { localPath: string; content: Buffer }>> {
  const files = new Map<string, { localPath: string; content: Buffer }>();
...
export async function syncToSpace(
  client: IsClient,
  localPath: string,
  spacePath: string,
  options: {
  ...
  let spaceFiles: Map<string, string>;
  try {
  ...
  if (!options.dryRun && result.newHead) {
    ...
    let newSpaceFiles: Map<string, string>;
    try {
...
function extractFrontmatter(content: string): { body: string; name?: string; summary?: string } {
